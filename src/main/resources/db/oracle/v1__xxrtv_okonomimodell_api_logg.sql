...
CREATE TABLE xxrtv.xxrtv_okonomimodell_api_logg(
    ...
    , response_oebs       CLOB                    -- Innholdet i svaret (kan være stort, derfor CLOB)
...
CREATE INDEX xxrtv.xxrtv_kalo_okonomimodell_api_U3 ON xxrtv.xxrtv_okonomimodell_api_logg
    (status) LOCAL;
...
CREATE  SEQUENCE xxrtv.xxrtv_okonomimodell_api_seq
    START WITH 1
    INCREMENT BY 1;
...
CREATE OR REPLACE TRIGGER xxrtv.xxrtv_kalo_okonomimodell_api_bir
  BEFORE INSERT ON xxrtv.xxrtv_okonomimodell_api_logg
  FOR EACH ROW
BEGIN
SELECT xxrtv.xxrtv_okonomimodell_api_seq.nextval
...
CREATE INDEX xxrtv.xxrtv_kalo_U1 ON xxrtv.xxrtv_okonomimodell_api_logg
    (operation, kall_retning);

-- Indeks på korrelasjon_id for rask oppslag når man skal finne alle logginnslag for ett kall.
CREATE INDEX xxrtv.xxrtv_kalo_U2 ON xxrtv.xxrtv_okonomimodell_api_logg
    (korrelasjon_id);
